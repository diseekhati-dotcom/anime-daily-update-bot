...
def add_chat(chat_id: int):
    ...
    try:
        ...
        log.exception("Could not save chat %s.", chat_id)
...
def remove_chat(chat_id: int):
    ...
    try:
        ...
        log.exception("Could not save removed chat %s.", chat_id)
...
async def register_chat(chat_id: int):
    await asyncio.to_thread(add_chat, chat_id)


async def unregister_chat(chat_id: int):
    await asyncio.to_thread(remove_chat, chat_id)


async def get_anime_message():
    result = await asyncio.to_thread(build_message)

    if inspect.isawaitable(result):
        result = await result

    if not isinstance(result, str) or not result.strip():
        raise RuntimeError("build_message() returned empty/invalid text.")
...
async def test(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await update.effective_message.reply_text(
            "⏳ Today's anime update is being prepared..."
        )
        msg = await get_anime_message()
        await context.bot.send_message(chat_id=chat.id, text=msg)
    except Exception:
        log.exception("Test send failed for %s", chat.id)
        await update.effective_message.reply_text(
            "❌ Update fetch/send failed. Check Render logs."
        )
